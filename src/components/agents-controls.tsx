  ...
  return (
    <div className="relative shrink-0" ref={wrapRef}>
      ...
      >
        {active ? (
          <>
            <span className="hidden text-[var(--text-faint)] sm:inline">Agent · </span>
            <span className="max-w-[5.5rem] truncate font-semibold text-[var(--accent)] sm:max-w-[8rem]">
              {active.name}
            </span>
          </>
        ) : (
      ...
      {open ? (
        <>
          ...
          <div
            ...
            role="dialog"
            aria-label="Agents"
            className={`${HEADER_PANEL_CLASS} w-[min(20rem,calc(100vw-1.5rem))] p-3 md:w-[min(20rem,calc(100vw-1.5rem))]`}
          >
            ...
            <div className="mt-3 flex gap-2 border-t border-white/[0.06] pt-2">
              <Link
                href="/settings?tab=agents"
                onClick={() => setOpen(false)}
                className="flex-1 rounded-full border border-white/[0.1] bg-white/[0.04] py-2 text-center text-[0.7rem] font-medium text-[var(--text-muted)] hover:text-[var(--text-primary)]"
              >
                Create agent
              </Link>
            </div>
